...
func ValidateToken() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		result, err := redis.DoRedisCommand("hget bookms_token_list token:user_id:"+strconv.Itoa(userID), "string")

		//如果redis没有token，则从数据库获取，重新存
		var info model.User
		if result == "" {
			database.DB.Model(&model.User{}).Where("id = ?", userID).First(&info)
			redis.DoRedisCommand("hset bookms_token_list token:user_id:"+strconv.Itoa(info.ID)+" "+info.Token, "int")
